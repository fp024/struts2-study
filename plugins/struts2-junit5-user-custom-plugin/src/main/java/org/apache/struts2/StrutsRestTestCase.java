...
public class StrutsRestTestCase<T> extends StrutsJUnit5TestCase<T> {
  ...
  /**
   * Executes an action and returns it's output (not the result returned from execute()), but the
   * actual output that would be written to the response. For this to work the configured result for
   * the action needs to be JSON, FreeMarker, or Velocity (JSPs can be used with the Embedded JSP
   * plugin)
   *
   ...
   * @throws UnsupportedEncodingException in case of unsupported encoding
   */
  protected String executeAction(String httpMethod, String uri)
  ...
  /**
   * Creates an action proxy for a request, and sets parameters of the ActionInvocation to the
   * passed parameters. Make sure to set the request parameters in the protected "request" object
